...
---
<p class= "aligncenter" >
  <img src="assets/images/thm-ra-logo.png" class="center">
</p>
...
We find installation files for spark 2.8.3 which is vulnerable (CVE-2020-12772). We get the NTLM hash 
...
### Enumerating SMB
Let's look at the smb shares and the password policy, looking at the password policy is good before doing any bruteforcing because if there is a account lock out policy set then we might end up locking out accounts and that's not what we want.
```
